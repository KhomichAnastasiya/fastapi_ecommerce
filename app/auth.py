...
def hash_password(password: str) -> str:
    """
    Converts the password to a hash using bcrypt.
    """
...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    """
    Checks whether the entered password matches the saved hash.
    """
...
def create_access_token(data: dict):
    """
    Creates jwt with payload (sub, role, id, exp). Access token.
    """
    to_encode = data.copy()
...
def create_refresh_token(data: dict):
    """
    Creates jwt with payload (sub, role, id, exp). Refresh token.
    """
    to_encode = data.copy()
...
async def get_current_user(token: str = Depends(oauth2_scheme),
    ...
    """
    Checks JWT and returns user from db.
    """
