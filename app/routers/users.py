...
async def create_user(user: UserCreate, db: AsyncSession = Depends(get_async_db)):
    """
    Registers a new user with the role of 'buyer' or 'seller
    """
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    """
    Authenticates the user and returns access token and refresh token.
    """
...
async def refresh_token(body: RefreshTokenRequest,
                        db: AsyncSession = Depends(get_async_db)):
    """
    Updates the refresh token by accepting the old refresh
    ...
    try:
        ...
        # the signature is incorrect or the token is corrupted
        raise credentials_exception
...
async def access_token(body: RefreshTokenRequest,
                        db: AsyncSession = Depends(get_async_db)):
    """
    Updates the access token by accepting the refresh
    token in the request body.
    """
    credentials_exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
